...
def test_agent_tool_no_parameter_conflict(agent, tool_registry, mock_randint):
    agent.tool_handler = unittest.mock.Mock(run_tool=unittest.mock.Mock(return_value=iter([])))
    ...
    agent.tool_handler.run_tool.assert_called_with(
        tool={
...
def test_agent_tool_with_name_normalization(agent, tool_registry, mock_randint):
    ...
    assert agent.tool_handler.run_tool.call_count == 1
    tool_call = agent.tool_handler.run_tool.call_args.kwargs.get("tool")
