...
from ..tools.executor import validate_and_prepare_tools, run_single_tool
from ..tools.executor_strategy import SequentialToolExecutorStrategy, ParallelToolExecutorStrategy, ToolExecutorStrategy
...
from ..types.tools import ToolConfig, ToolHandler, ToolResult, ToolUse, Tool
...
def event_loop_cycle(
    ...
    tool_executor_strategy: ToolExecutorStrategy,
    **kwargs: Any,
    ...
    Args:
        ...
        tool_executor_strategy: Optional handler for parallel tool execution.
    ...
    try:
        ...
        if stop_reason == "tool_use":
            ...
            tool_uses, tool_results, invalid_tool_use_ids = validate_and_prepare_tools(message)

            if not tool_uses:
                return stop_reason, message, event_loop_metrics, kwargs["request_state"]

            run_tool_handler = partial(
                run_single_tool,
            ...
            for succeeded, result in tool_executor_strategy.run_tools(tool_uses, run_tool_handler):
            ...
            return recurse_event_loop(
                ...
                tool_executor_strategy=tool_executor_strategy,
                **kwargs,
            )
